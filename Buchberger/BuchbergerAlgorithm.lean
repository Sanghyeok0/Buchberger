...
lemma leadingTerm_normalForm_not_mem_ideal_span_leadingTerm
  ...
  m.leadingTerm (normalForm m hG p) ∉ Ideal.span ((fun g => m.leadingTerm g) '' SetLike.coe G) := by
  classical
  ...
  have h_span_le :
      Ideal.span ((fun g => m.leadingTerm g) '' SetLike.coe G)
        ≤ monomialIdeal k (SetLike.coe (G.image m.degree)) := by
    ...
    have hLMg :
      ...
      have hα : m.degree g ∈ (SetLike.coe (G.image m.degree)) := by
  ...
  have hLT_r_in_mono :
      m.leadingTerm r ∈ monomialIdeal k (SetLike.coe (G.image m.degree)) :=

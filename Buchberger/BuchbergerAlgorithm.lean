...
lemma leadingTerm_normalForm_not_mem_ideal_span_leadingTerm
  ...
  m.leadingTerm (normalForm m hG p) ∉ Ideal.span ((fun g => m.leadingTerm g) '' G) := by
  ...
  have h_span_le :
      Ideal.span ((fun g => m.leadingTerm g) '' G)
        ≤ monomialIdeal k (↑(G.image m.degree)) := by
    ...
    have hLMg :
        monomial (m.degree g) (1 : k)
          ∈ monomialIdeal k (((G.image m.degree) : Set (σ →₀ ℕ))) := by
      -- 먼저 지수가 생성 집합에 들어감을 확보
      have hgG' : g ∈ G := by
        -- `hgG : g ∈ (↑G : Set _)` 형태라면 아래가 잘 먹습니다
        simpa using hgG
      have hα : m.degree g ∈ ((↑(G.image m.degree) : Set (σ →₀ ℕ))) := by
        -- finset 멤버십을 set 멤버십으로 바꾸기
        exact (Finset.mem_coe).2 (Finset.mem_image.2 ⟨g, hgG', rfl⟩)

      -- 이제 정의로 끝: 생성자는 span에 들어간다
      unfold monomialIdeal
      exact Ideal.subset_span ⟨m.degree g, hα, rfl⟩
    -- `LT g = C (lc g) * monomial (LM g) 1`, so it's in the ideal
    --simpa [leadingTerm, C_mul_monomial] using Ideal.mul_mem_left _ _ hLMg
    simp only [SetLike.mem_coe]
    unfold leadingTerm
    have : (monomial (m.degree g)) (m.leadingCoeff g)
      = C (m.leadingCoeff g) * (monomial (m.degree g) (1 : k)) := by rw [C_mul_monomial, mul_one]
    rw [this]
    exact Ideal.mul_mem_left _ _ hLMg

  -- Transport `LT r` into that monomial ideal
  have hLT_r_in_mono :
      m.leadingTerm r ∈ monomialIdeal k (G.image m.degree) :=
  ...
  have hLM_r_in_mono :
    ...
    simpa only [coe_image, this] using Ideal.mul_mem_left _ _ hLT_r_in_mono

  -- Divisibility in the monomial ideal ⇒ some `m.degree g ≤ m.degree r` with `g ∈ G`
  have hdiv :
      ∃ α ∈ (G.image m.degree), α ≤ m.degree r := by
    -- specialize `mem_ideal_span_monomial_image` to the monomial `x^(LM r)`
    have hLM_r_in_mono' : monomial (m.degree r) (1 : k)
        ∈ Ideal.span ((fun a => monomial a (1 : k)) '' (G.image m.degree)) := by
      simpa [monomialIdeal] using hLM_r_in_mono
    have h' :
        ∀ xi ∈ (monomial (m.degree r) (1 : k)).support,
          ∃ si ∈ (G.image m.degree), si ≤ xi := by
      -- note: this is exactly the ↔-direction of `mem_ideal_span_monomial_image`
      exact (mem_ideal_span_monomial_image
        (x := monomial (m.degree r) (1 : k))
        (s := (G.image m.degree))).1 hLM_r_in_mono'
    have hr_support : m.degree r ∈ (monomial (m.degree r) (1 : k)).support := by
      simp [MvPolynomial.support_monomial, one_ne_zero]
    -- since the support is a singleton, we only need the case `xi = m.degree r`
    simpa using h' (m.degree r) hr_support

  rcases hdiv with ⟨α, hα_in, hα_le⟩
  have hα_in' : α ∈ (G.image m.degree) := by
    simpa [Finset.mem_coe] using hα_in
  rcases Finset.mem_image.mp hα_in' with ⟨g, hgG, rfl⟩
  have h_remainder_prop := (normalForm_spec m hG p).2.2
  have h_deg_in_support : m.degree r ∈ r.support :=
    MonomialOrder.degree_mem_support (m := m) h_rem_ne_zero
  -- specialize the remainder property at the leading monomial of `r`
  specialize h_remainder_prop (m.degree r) h_deg_in_support
  -- now apply it to our specific `g ∈ G`
  have h_forbid : ¬ m.degree g ≤ m.degree r := h_remainder_prop g hgG
  exact h_forbid hα_le
...
noncomputable def NonZero_Rem_Spoly (G : Finset (MvPolynomial σ k))
  ...
  (G.offDiag.image (fun pq => normalForm m hG (sPolynomial m pq.1 pq.2))).filter (· ≠ 0)
...
theorem Buchberger_Alg [Finite σ]
  ...
  · let G := Buchberger_Step m hF
    ...
    intro g₁ hg₁ g₂ hg₂ hneq
    let r := normalForm m hG (sPolynomial m g₁ g₂)
...
theorem Buchberger_Alg_wf [Finite σ]
  ...
  let NonZero_Rem_Spoly_wf (s : {G : Finset (MvPolynomial σ k) // ∀ g ∈ G, g ≠ 0}) : Finset (MvPolynomial σ k) :=
    (s.val.offDiag.image (fun pq => normalForm m s.property (sPolynomial m pq.1 pq.2))).filter (· ≠ 0)
  ...
  have h_G_is_GB : IsGroebnerBasis m I G := by
    ...
    let r := normalForm m hG_nonzero (sPolynomial m p q)
    by_contra hr_nonzero

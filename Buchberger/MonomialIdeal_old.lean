...
variable (m) in
def initial_Ideal_new (I : Ideal (MvPolynomial σ R)) : Ideal (MvPolynomial σ R) :=
  monomialIdeal R (LM_set m I)

lemma leadingTermIdeal_is_initial_Ideal_new (I : Ideal (MvPolynomial σ k)) :
    leadingTermIdeal m I = initial_Ideal_new m I := by
  apply le_antisymm
  · -- Show span(LT) ⊆ span(LM)
    rw [initial_Ideal_new, leadingTermIdeal, Ideal.span_le]
    ...
    use g
  · -- Show span(LM) ⊆ span(LT)
    rw [initial_Ideal_new, monomialIdeal, leadingTermIdeal, Ideal.span_le]
    unfold LT_set LM_set
...
theorem Dickson_lemma_MV (S : Set (σ →₀ ℕ)) :
  ...
  · refine Ideal.span_le.mpr ?_
    ...
    have : Decidable ((1:R) = 0) := by exact Classical.propDecidable (1 = 0)

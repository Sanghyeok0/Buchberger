...
lemma minClasses_restrict_le_subset {N : Set M} {a : M} {_ : a ∈ N} :
  ...
  use d
  constructor
  · show Minimal (fun x ↦ x ∈ N) d
    simp only [Minimal]
    constructor
    · exact hdN
    · show ∀ ⦃y : M⦄, y ∈ N → y ≤ d → d ≤ y
      intro y hyN hle
      exact hmin' hyN (hle.trans hda) hle
  · rfl
  ...
  · -- Build a finite basis B ⊆ N
    ...
    · constructor
      ...
      · show ∀ a ∈ N, ∃ b ∈ B, b ≤ a
        ...
        have hγS : γ ∈ S := by
          have test : γ ∈ (minClasses N') := by exact pick' γ hγ_in_S'
          ...
          have sq : γ ∈ (minClasses N).toFinset := by exact Set.mem_toFinset.mpr (h_min_sub (pick' γ hγ_in_S'))
...
lemma wf_and_finite_antichains_implies_hasDicksonProperty :
    (WellFoundedLT M ∧ ∀ s : Set M, IsAntichain (· ≤ ·) s → s.Finite) → HasDicksonProperty M := by sorry

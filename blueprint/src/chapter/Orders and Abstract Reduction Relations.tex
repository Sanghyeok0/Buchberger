...
\begin{definition}[The ``$\le$–setoid'' on $M$]\label{def:eq_rel}
    \uses{def:Orders}
    \leanok %확인 필요
    \lean{leSetoid}
...
\begin{proposition}\label{prop:wqoEquivalent} % [Becker-Weispfenning1993] 160p Proposition 4.42
    ...
    \lean{wf_and_finite_antichains_implies_hasDicksonProperty, finite_min_classes_implies_hasDicksonProperty}
...
\begin{lemma}\label{lem:mem_monomialIdeal_iff_divisible} % [Cox] 70p Lemma 2
    \lean{MvPolynomial.mem_monomialIdeal_iff_divisible}
    \leanok %확인 필요
...
\begin{theorem}[Dickson's Lemma (MvPolynomial)]\label{thm:Dickson(MvPolynomial)}
    \uses{thm:Dickson}
    \lean{MvPolynomial.Dickson_lemma_MV}
    \leanok %확인 필요

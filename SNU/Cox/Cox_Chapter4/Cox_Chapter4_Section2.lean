...
/--
Cox–Little–O'Shea, Ch.4 §2, Definition 10.

If `f ∈ k[x₁, ..., xₙ]` is a polynomial, we define the reduction of `f`,
denoted `f_red`, to be the polynomial such that `⟨f_red⟩ = √⟨f⟩`.

A polynomial is said to be reduced (or square-free) if `f = f_red`.
-/
def IsReduction (f g : MvPolynomial σ k) : Prop :=
  Ideal.span ({g} : Set (MvPolynomial σ k))
    =
  (Ideal.span ({f} : Set (MvPolynomial σ k))).radical

/-- `f` is reduced (square-free) iff `⟨f⟩` is radical, i.e. `√⟨f⟩ = ⟨f⟩`. -/
def IsReduced (f : MvPolynomial σ k) : Prop :=
  (Ideal.span ({f} : Set (MvPolynomial σ k))).radical
    =
  Ideal.span ({f} : Set (MvPolynomial σ k))
